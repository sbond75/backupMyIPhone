...
if [[ "$useUSB" == 1 && -z "$nixShellToUseForUSB" ]]; then
    nixShellToUseForUSB='./shell_wifi_pair.nix'
elif [[ "$useUSB" != 1 && ! -z "$nixShellToUseForUSB" ]]; then #elif [[ "$useUSB" != 1 && ! -z "$ranWithTeeAlready" ]]; then # (ranWithTeeAlready sets nixShellToUseForUSB so we can't check for it being non-empty here due to the above if-statement not being true)
