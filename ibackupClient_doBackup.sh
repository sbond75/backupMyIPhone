...
function doBackup() {
	...
	if [ "$config__syncMethod" == "rsync_curlftpfs" ]; then
	    ...
	    echo rclone mount --no-check-certificate=true "myremote_$username:/" "$mountPoint" --vfs-cache-mode writes &
	    rclone mount --no-check-certificate=true "myremote_$username:/" "$mountPoint" --vfs-cache-mode writes &
